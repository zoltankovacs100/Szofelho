 ...
 * Description: QR → Login/Reg → LearnDash auto-enroll → Redirect a teszt oldalra. Token TTL, aláírás, marketing consent.
 * Version: 1.0.0
 * Author: Custom
 */
...
class QRAccessPlugin {
	...
	public function render_generate_page() {
		...
		$link = '';
		if (!empty($_POST['qr_generate']) && $course > 0 && filter_var($redirect, FILTER_VALIDATE_URL)) {
			$payload = [
				'cid' => $course,
				'redirect' => $redirect,
	...
	public function maybe_handle_qr_entry() {
		...
		// Mentünk UTM/campaign adatokat session szintjén
		if (!headers_sent()) {
			foreach (['utm_source','utm_medium','utm_campaign'] as $utm) {
				if (!empty($_GET[$utm])) {
					setcookie('qr_'.$utm, sanitize_text_field($_GET[$utm]), (int)$data['exp'], COOKIEPATH, COOKIE_DOMAIN, $secure, true);
				}
			}
		}
	...
	public function maybe_enroll_after_login($user_login, $user) {
		...
		foreach (['utm_source','utm_medium','utm_campaign'] as $utm) {
			if (!empty($_COOKIE['qr_'.$utm])) {
				update_user_meta($user->ID, 'qr_'.$utm, sanitize_text_field($_COOKIE['qr_'.$utm]));
			}
		}
